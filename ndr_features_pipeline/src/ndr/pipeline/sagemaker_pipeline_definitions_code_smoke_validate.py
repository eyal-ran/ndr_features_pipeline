...
"""SageMaker deployment pipeline definition for smoke validation of promoted artifacts."""
...
_VALIDATE_INPUT_DIR = "/opt/ml/processing/input/validation_report"
_VALIDATE_INPUT_FILE = f"{_VALIDATE_INPUT_DIR}/code_artifact_validate_report.json"
_SMOKE_OUTPUT_SOURCE_DIR = "/opt/ml/processing/output/smoke_report"
_SMOKE_OUTPUT_FILE = f"{_SMOKE_OUTPUT_SOURCE_DIR}/code_smoke_validate_report.json"
...
def build_code_smoke_validate_pipeline(
    pipeline_name: str,
    role_arn: str,
    default_bucket: str,
    region_name: str,
    ...
    processor = PySparkProcessor(
        base_job_name="ndr-code-smoke-validate",
    ...
    validate_contract = resolve_step_execution_contract(
        project_name=project_name_for_contracts,
        feature_spec_version=feature_spec_version_for_contracts,
        pipeline_job_name=_VALIDATE_PIPELINE_JOB_NAME,
        step_name=_VALIDATE_STEP_NAME,
    )
    smoke_contract = resolve_step_execution_contract(
        project_name=project_name_for_contracts,
        feature_spec_version=feature_spec_version_for_contracts,
        pipeline_job_name=PIPELINE_JOB_NAME,
        step_name=_SMOKE_STEP_NAME,
    )
    ...
    smoke_step = ProcessingStep(
        name=_SMOKE_STEP_NAME,
        processor=processor,
        code=smoke_contract.script_s3_uri,
        job_arguments=[
            "python",
            smoke_contract.entry_script,
            "--project-name",
            project_name,
            "--feature-spec-version",
            feature_spec_version,
        ...
        inputs=[
            ...
            ProcessingInput(
                source=validate_step.properties.ProcessingOutputConfig.Outputs[_VALIDATE_OUTPUT_NAME].S3Output.S3Uri,
                destination=_VALIDATE_INPUT_DIR,
                input_name="validation_report_input",
            ),
        ],
        outputs=[
            ProcessingOutput(
                output_name=_SMOKE_OUTPUT_NAME,
                source=_SMOKE_OUTPUT_SOURCE_DIR,
                destination=_handoff_s3_uri(
                    default_bucket=default_bucket,
                    pipeline_name=pipeline_name,
                    artifact_name="smoke_report",
                ),
            )
        ],
    ...
    return Pipeline(
        ...
        steps=[build_step, validate_step, smoke_step],
        sagemaker_session=session,
    )
