"""SageMaker pipeline definition for realtime dependency readiness artifacts."""
...
PIPELINE_JOB_NAME = "pipeline_rt_readiness"


def build_rt_readiness_pipeline(
    pipeline_name: str,
    role_arn: str,
    default_bucket: str,
    region_name: str,
    ...
    mlp = ParameterString(
        name="MlProjectName", default_value="<required:MlProjectName>"
    )
    batch = ParameterString(name="MiniBatchId", default_value="<required:MiniBatchId>")
    start = ParameterString(
        name="BatchStartTsIso", default_value="<required:BatchStartTsIso>"
    )
    end = ParameterString(
        name="BatchEndTsIso", default_value="<required:BatchEndTsIso>"
    )
    ...
    processor = PySparkProcessor(
        base_job_name="ndr-rt-readiness",
    ...
    contract = resolve_step_execution_contract(
        ...
        step_name="RtArtifactReadinessCheckerStep",
    )
    step = ProcessingStep(
        name="RtArtifactReadinessCheckerStep",
        processor=processor,
        code=contract.script_s3_uri,
        job_arguments=build_processing_step_launch_args(
            entry_script=contract.entry_script,
            module_name="ndr.scripts.run_rt_readiness_checker",
