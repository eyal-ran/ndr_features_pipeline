...
1) **Authoritative feature catalogs (model inputs + dependencies)**:
   ...
   - **Catalog counts (v1 expectations; generated by catalog builder and validated in CI):**
     - **Model-input features**
       - FG-A: **385** numeric features (38 metrics × 5 windows × 2 directions = 380, plus hour-of-day and 4 time flags).*
       - FG-C: **2,660** correlation features if using all 380 FG-A numeric metrics (7 derived features per metric).*
     - **Non-model fields**
       - Delta tables: **31 base metrics** + required keys per slice (schema contract).
       - FG-B host baselines: **3,420** stats for 380 FG-A metrics (9 stats per metric: median, p25, p75, p95, p99, mad, iqr, support_count, cold_start_flag) + metadata.
       - FG-B segment baselines: **3,420** stats for 380 FG-A metrics (same 9 stats per metric) + metadata.
     ...
     - *FG-C count explanation: 380 FG-A numeric metrics (76 metrics × 5 windows) × 7 derived correlation features (diff, ratio, z_mad, abs_dev_over_mad, z_mad_clipped, z_mad_signed_pow3, log_ratio) = 2,660.*
